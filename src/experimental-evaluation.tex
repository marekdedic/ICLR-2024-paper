...
The proposed methods were experimentally verified on 10 publicly available datasets, with the achitecture of the model identical accross them, the only difference being in the values of the hyper-parameters. The node2vec algorithm was used for generating the node embeddings, with an MLP classifier providing the predictions for node classification. The experiment was run \( 10 \) times end-to-end and results averaged. A detailed description of the used datasets and hyperparameter setup is available in Appendix \ref{sec:app:experiment-details}.
